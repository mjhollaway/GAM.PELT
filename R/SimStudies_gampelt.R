...
gen_t_all <- function(n_sites,nseg,SIM_SCEN){
  ...
  for (curr_t_seg in 1:nseg){
    ...
    if (t_cng_samp[curr_t_seg] == 'no_cng'){
      ...
      t_AR_seg_all[,curr_t_seg] <- msm::rtnorm(n_sites, lower=0.1, upper=0.9)
      if (curr_t_seg == 1){
      ...
      t_AR_seg_all[site_cng,curr_t_seg]   <- msm::rtnorm(1, lower=0.1, upper=0.9)
...
gen_t_struct <- function(SIM_SCEN,n_sites,nseg){
  ...
  if (SIM_SCEN == '1'){
    ...
    t_AR_seg   <- matrix(rep(msm::rtnorm(1, lower=0.1, upper=0.9),n_sites*nseg),ncol=nseg,nrow=n_sites)
    ...
    t_AR_seg   <- matrix(rep(msm::rtnorm(n_sites, lower=0.1, upper=0.9),nseg),ncol=nseg,nrow=n_sites)
    t_var_seg  <- matrix(rep(1.0,n_sites*nseg),ncol=nseg,nrow=n_sites)
    t_mean_seg <- matrix(rep(0.0,n_sites*nseg),ncol=nseg,nrow=n_sites)
    ...
    t_AR_seg  <- matrix(msm::rtnorm(nseg*n_sites, lower=0.1, upper=0.9),ncol=nseg,nrow=n_sites)
    ...
    t_AR_seg[,1]   <- rep(msm::rtnorm(1, lower=0.1, upper=0.9),n_sites)
    t_var_seg[,1]  <- 1.0
    t_mean_seg[,1] <- 0.0
    ...
    for (ii in 2:nseg){
      ...
      t_AR_seg[site_cng,ii]   <- msm::rtnorm(1, lower=0.1, upper=0.9)
...
gen_SP_corr <- function(vgm_sill, vgm_rng, vgm_nmax, loc_pts, nseg){
  ...
  spat_model <- gstat::gstat(formula=z~1, ## We assume that there is a constant trend in the data
                             locations=~x+y,
                             dummy=T,    ## Logical value to set to True for unconditional simulation
                             beta=0.0,
                             model=gstat::vgm(psill=vgm_sill,range=vgm_rng,nugget=0.0,model='Sph'),
                             nmax=vgm_nmax)
...
#' Generate a test simulation dataset.
#'
#' @description
#' This functions generates a simulated dataset based on the specified scenario. Users are able to
#' generate simulated datasets for various changes in the spatial, temporal or spatio-temporal structure
#' of the dataset. See the GAM-PELT paper for more details.
#'
#'
#' @param SCENARIO Chosen scenario, will be one of either 1a, 1b, 1c, 2a, 2b, 2c (temporal changes only),
#' 3a, 3b, 3c (spatial changes only), 4a, 4b (spatio-temporal changes), A, B, C, D, E, F (no changes).
#' @param seed Random seed to use for reproducibility.
#' @param n_ts  Number of timesteps in dataset. Default is 500.
#' @param n_sites Number of spatial locations in dataset. Default is 50
#' @param true_cpts Locations of the true changepoints in the dataset. Default is 100, 200, 300, and 400 when n_ts=500.
#'
#' @return A list with the elements:
#' @return \code{data} - A data frame containing the data in the correct format to run \code{GAM-PELT}. This contains
#'              the model response variable (Y) and covariates which by default are the spatial
#'              locations of the data (U,V) and the time index (T).
#' @return \code{true_cpts}  - The locations of the true changepoints in the dataset.
#' @return \code{sp_seg_cng} - The type of spatial change between changepoints (i.e. for each segment)
#' @return \code{t_seg_cng}  - The type of temporal change between changepoints (i.e. for each segment)
#'
...
gen_SimData <- function(SCENARIO,seed,n_ts=500,n_sites=50,true_cpts=c(100,200,300,400)){
  ...
  return(list(data=df_sim$df_out,true_cpts=true_cpts,sp_seg_cng=df_sim$sp_seg_cng,t_seg_cng=df_sim$t_seg_cng))
